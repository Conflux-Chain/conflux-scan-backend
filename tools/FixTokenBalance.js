...
exports.updateTokenBalancesDynamic = updateTokenBalancesDynamic;
exports.updateTokenBalancesWithResume = updateTokenBalancesWithResume;
exports.updateTokenBalancesUltimate = updateTokenBalancesUltimate;
const sequelize_1 = require("sequelize");
const Token_1 = require("../stat/model/Token");
const Balance_1 = require("../stat/model/Balance");
const FixDailyTokenStat_1 = require("../stat/service/tool/FixDailyTokenStat");
/**
 * 获取下一个要处理的合约ID
 */
async function getNextContractId(lastContractId) {
    const nextContract = await Token_1.Erc1155Data.findOne({
        attributes: ['contractId'],
        where: {
            contractId: { [sequelize_1.Op.gt]: lastContractId }
        },
 ...
 * 获取下一个要处理的地址ID
 */
async function getNextAddressId(contractId, lastAddressId) {
    const nextAddress = await Token_1.Erc1155Data.findOne({
        attributes: ['addressId'],
        where: {
            contractId,
            addressId: { [sequelize_1.Op.gt]: lastAddressId }
        },
        order: [['addressId', 'ASC']],
        raw: true
    });
 ...
 * 获取指定合约和地址的记录数
 */
async function getRecordCount(contractId, addressId, transaction) {
    const count = await Token_1.Erc1155Data.count({
 ...
 * 查询现有的TokenBalance记录
 */
async function findExistingTokenBalance(contractId, addressId, transaction) {
    return await Balance_1.TokenBalance.findOne({
        where: { contractId, addressId },
        transaction
    });
}
 ...
 * 插入新的TokenBalance记录
 */
async function insertTokenBalance(contractId, addressId, balance, transaction) {
    await Balance_1.TokenBalance.create({
        contractId,
        addressId,
        balance
    }, { transaction });
 ...
 * 更新TokenBalance记录
 */
async function updateTokenBalance(tokenBalance, newBalance, oldBalance, transaction) {
 ...
 * 处理单个合约+地址组合
 */
async function processAddressPair(contractId, addressId, stats) {
 ...
 * 处理单个合约下的所有地址
 */
async function processContract(contractId, stats) {
 ...
 * 主处理函数 - 使用动态游标方式
 */
async function updateTokenBalancesDynamic() {
    const stats = {
        totalContracts: 0,
        totalAddresses: 0,
        totalRecords: 0,
 ...
 * 带断点续传功能的版本
 */
async function updateTokenBalancesWithResume(resumeFrom) {
    const stats = {
        totalContracts: 0,
        totalAddresses: 0,
        totalRecords: 0,
 ...
 * 极致优化版本 - 使用原生SQL
 */
async function updateTokenBalancesUltimate() {
    const stats = {
        totalContracts: 0,
        totalAddresses: 0,
        totalRecords: 0,
    ...
    try {
        while (true) {
            // 使用原生SQL获取下一个要处理的组合
            const [nextPair] = await sequelize.query(`
 ...
 */
async function main() {
    await (0, FixDailyTokenStat_1.init)();
    sequelize = Balance_1.TokenBalance.sequelize;
    try {
