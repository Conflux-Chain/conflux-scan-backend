...
export async function initRateLimiters(redisConf) {
    ...
    rateLimiterFree = new RateLimiterRedis({
        storeClient: redisClient,
        points: 5,
        duration: 1,
        keyPrefix: `api-${LEVEL_FREE}`,
    });
    rateLimiterStandard = new RateLimiterRedis({
        storeClient: redisClient,
        points: 20,
        duration: 1,
        keyPrefix: `api-${LEVEL_STANDARD}`,
    });
    rateLimiterEnterprise = new RateLimiterRedis({
        storeClient: redisClient,
        points: 100,
        duration: 1,
...
export async function checkRateByLevel(ctx, next) {
    ...
    try {
        ...
        await rateLimiter.consume(rateKey).catch(e => {e.message = 'limited';throw e;});
        rateLimiterDaily && (await rateLimiterDaily.consume(rateKey).catch(e => {e.message = 'limitedDaily';throw e;}));

    } catch (e) {
        let msg = e.message;
        if (msg === 'limited') {
            msg = `Too many requests. Allow ${rateLimiter["points"]}/s`;
        } else if (msg === 'limitedDaily') {
            msg = `Too many requests. Allow ${rateLimiterDaily["points"]}/day`;
        }
        ctx.body = {code: 429, message: msg};
        console.log(`rateId ${rateId} paid ${paid} level ${level} rlt ${JSON.stringify(e)} msg ${JSON.stringify(msg)}`);
        return;
    }
...
async function checkRateByAddress0(addressParamName, ctx, next) {
    ...
    try {
        ...
        ctx.body = {code: 429, message: `Too many requests. Allow ${rateLimiterAddress["points"]}/s`};
        return;
    }
...
export async function checkApiKeyByLevel(path, apiKey: string) {
    ...
    if (rateKey) {
        return {
            ok: rateKey.expireAt.getTime() > Date.now() ? true : false,
            result: {
                vipInfo: {keys: ['level'], values: [rateKey.level]},
