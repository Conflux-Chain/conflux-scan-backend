...
export async function patchApprovalList({cfx, account, list} =
    ...
    list.forEach(entry => {
        ...
        //@ts-ignore
        const {balanceOf, getApproved, isApprovedForAll} = new Contract({abi, address: entry.base32}, cfx)
        if (entry.type === 'ERC20') {
            tasks.push(balanceOf(account).then(res => {
                entry.currentApproval = res.toString();
            }).catch(e => setError(entry, e)));
        } else if (entry.approvalType === "ApprovalForAll") {
            tasks.push(isApprovedForAll(account, entry.spender).then(res => {
                entry.currentApproval = res.toString();
            }).catch(e => setError(entry, e)));
        } else if (entry.type === "ERC721") {
            tasks.push(getApproved(entry.spender).then(res => {
                entry.currentApproval = res.toString();
            }).catch(e => setError(entry, e)));
        }
    })
