...
export interface IUniqueAddrDaily {
	id?:number
	timeStart: Date
	timeEnd: Date
	contractId:number
...
export class UniqueAddressDaily extends Model<IUniqueAddrHourly> implements IUniqueAddrDaily {
	id?:number
	timeStart: Date
	// main prop
	contractId:number
	...
	static register(seq:Sequelize) {
		UniqueAddressDaily.init({
			...
			sequelize: seq, tableName: 'unique_addr_Daily',
			indexes: [
				{name: 'uk_epoch_cid_addr', unique: true, fields:['timeStart','contractId', 'addr'], },
			]
		})
