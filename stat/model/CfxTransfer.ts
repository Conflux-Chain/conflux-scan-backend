...
export function buildAddressCfxTransferList(list:any[]) : IAddressCfxTransfer[] {
    const result : any[] = []
    let idx = 0
    list.forEach(row=>{
...
export async function pagingFullCfxTransfer(skip:number, logger: any) : Promise<CfxTransferPage> {
    // find the max mark
