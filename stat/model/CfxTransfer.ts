...
// ============= partition by address table, transfer with balance ==============
export interface IAddressCfxBill {
    addressId: number
    epoch: number
    tracePos: number
    createdAt: Date
...
export const T_ADDRESS_CFX_BILL = 'address_cfx_bill'
export const T_ADDRESS_CFX_BILL_SQL = `
    create table if not exists ${T_ADDRESS_CFX_BILL} (
...
partition by hash (addressId)
  PARTITIONS 97;
`
export async function createAddressCfxBillTable(seq:Sequelize) {
    return createTable(seq, T_ADDRESS_CFX_BILL_SQL)
        .then(()=>{
            return AddressCfxBill.register(seq)
        }).then(()=>{
            AddressCfxBill.removeAttribute("id")
        }).catch(err=>{
            console.log(`createAddressCfxTransferTable fail, sql ${T_ADDRESS_CFX_BILL_SQL}:`, err)
            process.exit(9)
        })
}
export class AddressCfxBill extends Model<IAddressCfxBill> implements IAddressCfxBill {
    addressId: number
    epoch: number
    tracePos: number
    createdAt: Date
        ...
        AddressCfxBill.init(
            {
                ...
                balance: {type: DataTypes.DECIMAL(36, 0), allowNull: false},
            }, {
                sequelize: seq,
                updatedAt: false,
                tableName: T_ADDRESS_CFX_BILL,
                indexes: [
                    {
                        name: 'idx_epoch',
