...
BOUNDARY_OID_FILE="$(mktemp)"
PUSH_REFSPECS="$(mktemp)"
PUSH_LEASES="$(mktemp)"

cleanup() {
    rm -f "${REMOTE_TAGS}" "${NORMALIZED_TAGS}" "${TAGS_TO_DELETE}" "${BOUNDARY_OID_FILE}" "${PUSH_REFSPECS}" "${PUSH_LEASES}"
}
trap cleanup EXIT
...
awk -v boundary="${BOUNDARY_TAG}" '
    $1 == boundary { found=1; print $2 > boundaryOidFile }
    !found { print }
    END {
        if (!found) {
...
' boundaryOidFile="${BOUNDARY_OID_FILE}" "${NORMALIZED_TAGS}" \
> "${TAGS_TO_DELETE}"

if [[ ! -s "${TAGS_TO_DELETE}" ]]; then
    echo "No tags older than ${BOUNDARY_TAG} to delete."
    exit 0
fi

BOUNDARY_OID="$(cat "${BOUNDARY_OID_FILE}")"
git fetch --no-tags "${REMOTE}" "refs/tags/${BOUNDARY_TAG}"
printf '%s\n' "--force-with-lease=refs/tags/${BOUNDARY_TAG}:${BOUNDARY_OID}" >> "${PUSH_LEASES}"
printf '%s\n' "${BOUNDARY_OID}:refs/tags/${BOUNDARY_TAG}" >> "${PUSH_REFSPECS}"
