...
import {mustBeAddressParamIfPresent, mustBeHex64ParamIfPresent, getCfxSdk, InvalidParamError} from "../../stat/service/common/utils";
...
function assertSkipWithinLimit(skip: number) {
    if (skip >= SKIP_MAX) {
        throw new InvalidParamError(`Parameter <skip> exceeds listLimit (${SKIP_MAX})`);
    }
}
...
export async function getAATxDetail(ctx) {
    ...
    if (!userOpHash) {
        throw new InvalidParamError('param <userOpHash> is required');
    }
...
export async function getBundleTxDetail(ctx) {
    ...
    if (!txHash) {
        throw new InvalidParamError('param <txHash> is required');
    }
    ...
    if (!result) {
        throw new InvalidParamError(`Bundle tx not found or not a 4337 bundle: ${txHash}`);
    }
